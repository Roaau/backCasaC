...
<div style="padding:40px 16px;">
  <div style="max-width:480px;margin:0 auto;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
    ...
    <div style="background:#ffffff;padding:40px;">
      ...
      <div style="background:#f8fffe;border:2px solid #4cd96f;border-radius:12px;
                  padding:24px;text-align:center;margin-bottom:28px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:700;color:#888;
                  text-transform:uppercase;letter-spacing:2px;">Código de verificación</p>
        <p style="margin:0;font-family:'Courier New',monospace;font-size:42px;
                  font-weight:800;color:#1e2227;letter-spacing:10px;">${codigo}</p>
      </div>

      <div style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 8px 8px 0;
                  padding:12px 16px;margin-bottom:20px;">
        <p style="margin:0;font-size:13px;color:#78350f;">
          ⏱ Este código expira en <strong>15 minutos</strong> y es de un solo uso.
        </p>
      </div>
...
<div style="padding:40px 16px;">
  ...
  await transporter.sendMail({ from, to: destinatario, subject: 'Restablece tu contraseña — SC POS', html });
};
  ...
  <div style="max-width:480px;margin:0 auto;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
    ...
    <div style="background:#fff;padding:40px;">
      ...
      <p style="margin:20px 0 0;font-size:13px;color:#888;">
        Si crees que hay un error o quieres más información, contáctanos respondiendo este correo.
      </p>
    </div>
    <div style="background:#f8f9fa;border-top:1px solid #e8e8e8;padding:18px 40px;text-align:center;">
      <p style="margin:0;font-size:12px;color:#aaa;">SC POS · Sistema de Punto de Venta</p>
...
export const enviarFacturaPorCorreo = async ({
  ...
  // Logo como adjunto inline (CID) — compatible con Gmail, Outlook, Apple Mail
  const attachments = [];
  let logoHtml = "";
  if (logo_b64) {
    ...
    logoHtml = `<img src="cid:negocio-logo"
      style="height:56px;width:auto;max-width:200px;object-fit:contain;
             display:block;margin:0 auto 14px;">`;
  }

  const fecha = new Date(cfdi.fecha_venta).toLocaleDateString("es-MX", {
    weekday: "long", day: "2-digit", month: "long", year: "numeric",
  });
  const total = Number(cfdi.total_venta).toLocaleString("es-MX", {
...
<title>Factura ${cfdi.folio_venta}</title>
</head>
<body style="margin:0;padding:0;background:#f0f2f5;font-family:'Helvetica Neue',Arial,sans-serif;">
...
<div style="max-width:600px;margin:0 auto;border-radius:12px;overflow:hidden;
  ...
  <div style="background:#1e2227;padding:32px 40px;text-align:center;">
    ...
    <p style="margin:6px 0 0;color:#9aa0a6;font-size:13px;letter-spacing:0.3px;">
  ...
  <div style="background:#ffffff;padding:36px 40px;">
    ...
    <!-- Sección: Comprobante -->
    <p style="margin:0 0 10px;font-size:11px;font-weight:700;color:#9aa0a6;
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:28px;">
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;width:45%;border-bottom:1px solid #f5f5f5;">Folio</td>
        <td style="padding:9px 0;font-size:14px;font-weight:700;color:#1a1a1a;text-align:right;border-bottom:1px solid #f5f5f5;">${cfdi.folio_venta}</td>
      </tr>
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;border-bottom:1px solid #f5f5f5;">Fecha de emisión</td>
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#1a1a1a;text-align:right;border-bottom:1px solid #f5f5f5;">${fecha}</td>
      </tr>
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;">Uso CFDI</td>
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#1a1a1a;text-align:right;">${cfdi.uso_cfdi}</td>
      </tr>
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:28px;">
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;width:45%;border-bottom:1px solid #f5f5f5;">RFC</td>
        <td style="padding:9px 0;font-size:14px;font-weight:700;color:#1a1a1a;text-align:right;font-family:monospace;border-bottom:1px solid #f5f5f5;">${cfdi.receptor_rfc}</td>
      </tr>
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;border-bottom:1px solid #f5f5f5;">Razón Social</td>
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#1a1a1a;text-align:right;border-bottom:1px solid #f5f5f5;">${cfdi.receptor_nombre}</td>
      </tr>
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;border-bottom:1px solid #f5f5f5;">CP Fiscal</td>
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#1a1a1a;text-align:right;border-bottom:1px solid #f5f5f5;">${cfdi.receptor_cp}</td>
      </tr>
      <tr>
        <td style="padding:9px 0;font-size:14px;color:#888;">Régimen Fiscal</td>
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#1a1a1a;text-align:right;">${cfdi.receptor_regimen}</td>
      </tr>
    </table>
    ...
    <div style="background:#f8f9fa;border:1px dashed #d0d0d0;border-radius:8px;
      ...
      <p style="margin:0;font-family:'Courier New',monospace;font-size:13px;
                color:#333;word-break:break-all;font-weight:600;">${cfdi.cfdi_uuid}</p>
    </div>
    ...
    <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:10px;
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="font-size:15px;color:#166534;font-weight:600;padding:0;">Total Facturado</td>
          <td style="font-size:28px;font-weight:800;color:#15803d;text-align:right;padding:0;">${total}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 8px 8px 0;
      ...
      <p style="margin:0;font-size:13px;color:#78350f;line-height:1.6;">
        <strong>📎 Conserve este correo como comprobante fiscal.</strong><br>
        Este documento confirma el timbrado del CFDI ante el SAT.
        El XML oficial estará disponible a la brevedad.
      </p>
    </div>
  ...
  <!-- PIE DE PÁGINA -->
  <div style="background:#f8f9fa;border-top:1px solid #e8e8e8;
              padding:22px 40px;text-align:center;">
    <p style="margin:0 0 4px;font-size:13px;color:#555;">
      Este comprobante fue emitido por <strong>${negocio}</strong>
    </p>
