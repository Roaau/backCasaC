...
describe('Caja — estadoCaja()', () => {
  ...
  test('devuelve abierta: false cuando no hay caja activa', async () => {
    ...
    const req = { query: { usuario_id: '1', sucursal_id: '1' }, usuario: { empresa_id: 1 } };
    const res = mockRes();
...
describe('Caja — abrirCaja()', () => {
  ...
  test('devuelve 400 si ya hay una caja abierta hoy', async () => {
    ...
    const req = { body: { usuario_id: 1, monto: 500, sucursal_id: 1 }, usuario: { empresa_id: 1 } };
    const res = mockRes();
  ...
  test('crea una caja nueva si no hay caja activa', async () => {
    ...
    const req = { body: { usuario_id: 1, monto: 300, sucursal_id: 1 }, usuario: { empresa_id: 1, idUsuario: 1 } };
    const res = mockRes();
    await abrirCaja(req, res);
